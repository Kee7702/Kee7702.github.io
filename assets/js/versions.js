...
versionname.appendChild(versioncodes);if(versionlist[version].codes['armeabi-v7a']){var versioncodearmeabiv7a=document.createElement('a');versioncodearmeabiv7a.textContent=("armeabi-v7a: "+versionlist[version].codes['armeabi-v7a'])
...
if(versionlist[version].codes.x86){var versioncodex86=document.createElement('a');versioncodex86.textContent=("x86: "+versionlist[version].codes.x86)
...
if(versionlist[version].codes['arm64-v8a']){var versioncodearm64v8a=document.createElement('a');versioncodearm64v8a.textContent=("arm64-v8a: "+versionlist[version].codes['arm64-v8a'])
...
if(versionlist[version].codes.x86_64){var versioncodex8664=document.createElement('a');versioncodex8664.textContent=("x86_64: "+versionlist[version].codes.x86_64)
...
while(versionlist[version+1]);})();(async()=>{const res=await fetch(`/app/tbversion.json`);const json=await res.json();versionlist=(json);version=-1
do{version++
var versionicon=document.createElement('i');versionicon.className="material-icons";versionicon.textContent="update"
var versiontitle=document.createElement('a');versiontitle.textContent=versionlist[version].version_name
var versionbeta=null
