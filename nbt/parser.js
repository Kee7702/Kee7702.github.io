...
for (const rr of vars) {
  if (rr.length > 0) {
    insert += '<p style="margin:0">' + rr + '</p><input type="text" placeholder="' + rr + '" name="' + rr + '" required="">';
    script += "prexport=prexport.replace('<!presetVar=" + rr + "!>',document.getElementsByName('" + rr + "')[0].value);"
  }
};
document.getElementsByTagName('p')[0].outerHTML = insert + '<button onclick="presetExport()">Export</button>';
a = document.createElement('script');
a.innerHTML = 'function presetExport() {prexport=preset;' + script + 'const link=document.createElement(\'a\');const content=prexport;const file=new Blob([content], {type:\'text/plain\'});link.href=URL.createObjectURL(file);link.download=\'' + document.title.split('-')[0].replaceAll(' ', '')+"-" + '\'+new Date().valueOf().toString(36)+\'.txt\';link.click();URL.revokeObjectURL(link.href)}';
